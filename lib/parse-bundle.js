// @source https://github.com/webpack-contrib/webpack-bundle-analyzer/blob/939ddc816b03cab70e3693dc943876cc48489014/src/parseUtils.js
...
function isWebpack5SetupFunction(node) {
  const modulesNode = _.get(node, 'body.body[0].argument.callee.body.body[1].declarations[0].init');

  return modulesNode.type === 'ObjectExpression' && modulesNode.properties.length;
}
...
function getModulesLocations(node) {
  ...
  if (node.type === 'FunctionExpression') {
    // TODO: add support to builtin Webpack modules like
    // webpack/runtime/define property getters or webpack/runtime/make namespace object
    // They're defined outside user modules map, as named properties of __webpack_require__ function
    const modulesNode = node.body.body[0].argument.callee.body.body[1].declarations[0].init;

    if (modulesNode.type === 'ObjectExpression') {
      return getModulesLocations(modulesNode);
    }
  }
  ...
  walk.recursive(
    ...
    walkState,
    {
      ...
      CallExpression(node, state, c) {
        ...
        // Both globalObject and chunkCallbackName can be changed through the config,
        ...
        if (
          ...
          && !node.callee.id
          && args.length === 2
          && isWebpack5SetupFunction(args[1])
        ) {
          state.locations = getModulesLocations(args[1]);
          return;
        }
