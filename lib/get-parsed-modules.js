...
const { getParsedIdentifer } = require('./identifier');
...
function setParsedModules({
  ...
  for (const chunk of chunks) {
    for (const file of chunk.files) {
      ...
      const bundleSource = assets[file].source();
      const { modules } = parseBundle(bundleSource);

      let moduleIndex = 0;
      for (const module of chunk.modulesIterable) {
        const identifier = getParsedIdentifer(module.identifier(), context);

        const moduleSource = modules[moduleIndex];
